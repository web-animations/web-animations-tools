...
function serializeTimedItem(timedItem, indent) {
  ...
  function keyframeEffectToJs(effect) {
    var needsConstructor = effect.composite != 'replace' && effect.accumulate != 'none';
    var result = '';
    if (needsConstructor) {
  ...
  function timingToJs(timing) {
    ...
    if (!isMultiLine(timing) && timing.duration != null) {
  ...
  function elementToJs(element) {
    ...
    if (element.id !== undefined) {
      return 'document.querySelector(' + valueToJsLiteral('#' + String(element.id)) + ')';
    }
