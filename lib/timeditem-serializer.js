...
function serializeTimedItem(timedItem, indent) {
  ...
  function timedItemToJs(timedItem) {
    ...
    } else if (timedItem instanceof AnimationSequence) {
      return groupToJs('AnimationSequence', timedItem);
    } else if (timedItem instanceof AnimationGroup) {
      return groupToJs('AnimationGroup', timedItem);
    }
  }
  function animationToJs(animation) {
    ...
    result += timingToJs(animation.timing) + ')';
    return result;
  }
  function groupToJs(constructor, group) {
    ...
    var timing = timingToJs(group.timing);
    if (children != '[]' || timing != '{}') {
      result += children;
      if (timing != '{}') {
        result += ', ' + timing;
